...
public class Adresse {
	...
	/**
     * Redéfinition de la méthode equals pour comparer les adresses par numéro, rue, code postal et ville.
     * @param autreAdresse l'autre adresse à comparer
     * @return vrai si les adresses sont égales (même numéro, même rue, même code postal et même ville), sinon faux.
     */
    @Override
    public boolean equals(Object autreAdresse) {
        if (this == autreAdresse) {
            return true;
        }
        if (autreAdresse == null || getClass() != autreAdresse.getClass()) {
            return false;
        }

        Adresse adresse = (Adresse) autreAdresse;

        // Comparaison du numéro, de la rue, du code postal et de la ville
        return (numero == adresse.numero) &&
         (rue.equals(adresse.rue)) &&
         (code_postal.equals(adresse.code_postal)) &&
